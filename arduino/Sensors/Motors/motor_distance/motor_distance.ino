...
// The direction of the sensors are from the back of the robot
...
MazebusterIR irFrontLeft(SENSOR_IR_FRONT_LEFT, 12, 25, 93);
MazebusterIR irFrontRight(SENSOR_IR_FRONT_RIGHT, 11, 25, 93);
...
void adjustRobotPosition() {  //Adjust the position of robot, so that it won't hit the obstacle  
  //There's an obstacle at front-left 10cm position from robot, but no obstacle right in front
  if(sensorReadings[0] < 20) {
    boolean senseOrg = sense;
    
    start = B1;         //Start motor
    dir = B10;          //Turn right
    travelDist = 0;
    angle = 90;         //90 degree
    configMove();
    while(motorLRun || motorRRun) {
      controlRobot();
      delay(50);
    }
    
    start = B1;         //Start motor
    dir = B00;          //Move forward
    travelDist = 60;    //Adjust to move 2cm forward
    angle = 0;
    configMove();
    while(motorLRun || motorRRun) {
      controlRobot();
...
//  int gap, targetGap;
//  int inclineStatus = 0;    //0 = No inclination, 1 = Left inclination, 2 = Right inclination
//  
//  gap = irLeft.distance();
//  if(gap >= 6 && gap <= 75) {                                //Left sensor within detection range
//    gap = irLeft.distance();                //Left gap distance to obstacle/wall
//    targetGap = round((gap-1)/10.0) * 10 + 1;                //Target left gap distance    
//    if(gap >= (targetGap-5) && gap < (targetGap-1)) {        //Robot inclines to the left
//      inclineStatus = 1;
//    }
//    else if(gap > (targetGap+1) && gap <= (targetGap+4)) {   //Robot inclines to the right
//     inclineStatus = 2;
//    }
//    else inclineStatus = 0;
//  }
//  else if(gap > 75) {                                        //Right sensor within detection range
//    gap = irRight.distance();                //Right gap distance to obstacle/wall
//    targetGap = round((gap-1)/10.0) * 10 + 1;                //Target left gap distance
//    if(gap >= (targetGap-5) && gap < (targetGap-1)) {        //Robot inclines to the left
//      inclineStatus = 2;
//    }
...
//    else inclineStatus = 0;
//  }
//  else inclineStatus = 1;   //Too near to the left wall
//  
//  motorSpeedPID.Compute();  //Compute the target speed of motors based on the current encoder ticks count w.r.t the checkpoint
//  
//  if(dir == 0x00 || dir == 0x03) {  //Apply only when the robot is moving forward or backward
//    if(inclineStatus == 1) {        //Robot inclines to the left
//      motorLPID.SetTunings(motorLPID.GetKp(), motorLPID.GetKi()*1.005, motorLPID.GetKd());
//      motorRPID.SetTunings(motorRPID.GetKp(), motorRPID.GetKi()*0.995, motorRPID.GetKd());
//      targetLSpeed = targetMotorSpeed * 1.005;  //Increase left motor speed
//      targetRSpeed = targetMotorSpeed * 0.995;  //Decrease right motor speed
//      //motorLPID.SetOutputLimits(80, min(motorRPWM*1.01, 255));
//      //motorRPID.SetOutputLimits(82, max(motorRPWM*0.99, 82));
//    }
//    else if(inclineStatus == 2) {   //Robot inclines to the right
//      motorLPID.SetTunings(motorLPID.GetKp(), motorLPID.GetKi()*0.995, motorLPID.GetKd());
//      motorRPID.SetTunings(motorRPID.GetKp(), motorRPID.GetKi()*1.005, motorRPID.GetKd());
//      targetLSpeed = targetMotorSpeed * 0.995;  //Decrease left motor speed
//      targetRSpeed = targetMotorSpeed * 1.005;  //Increase right motor speed
//      //motorLPID.SetOutputLimits(80, max(motorLPWM*0.99, 80));
//      //motorRPID.SetOutputLimits(82, min(motorLPWM*1.01, 255));
//    }
//    else {
